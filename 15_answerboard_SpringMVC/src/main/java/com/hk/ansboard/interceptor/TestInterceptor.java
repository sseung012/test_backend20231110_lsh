...
//해당 클래스를 인터셉터로 설정하려면 HandlerInterceptor
@Component
public class TestInterceptor implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(request.getRequestURI().contains("/boardList.do")) {
			...
			System.out.println("글 목록을 요청했습니다.");
			return true;
		}else if(request.getRequestURI().contains("/detailBoard.do")) {
			if(obj==null) {
				System.out.println("글 상세조회는 로그인 한 후에 조회 가능 합니다");
				logger.info(request.getRequestURI()+"글 상세조회");
//				response.sendRedirect("index.jsp");
	...
	//컨트롤러 실행 후 DispatcherServlet이 뷰로 보내기 전에 호출됌
	@Override
