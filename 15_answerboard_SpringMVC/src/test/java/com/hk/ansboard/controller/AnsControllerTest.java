...
public class AnsControllerTest {
   ...
   public void testGetAllList() throws Exception {
      ...
      //boardList.do를 요청해보자
      MvcResult result=mock.perform(MockMvcRequestBuilders.get("/boardList.do").param("pnum", "1"))
			    		  .andExpect(MockMvcResultMatchers.status().isOk())
			    		  .andReturn();
	      
      int statusCode=result.getResponse().getStatus(); //결과내용을 받아옴
      System.out.println("status 코드 : " + statusCode);
      System.out.println("HttpStatus 코드 : " + HttpStatus.OK.value());
      //200코드와 같은지를 비교하기 --> 200은 성공, 404는 경로에러, 500은 코드 오류
      assertEquals(HttpStatus.OK.value(), statusCode);      
   }
   
   //detailBoard.do 글 상세보기
   @Test
   public void testDetailBoard() throws Exception {
	   ...
	   //boardList.do를 요청해보자
	   MvcResult result=mock.perform(MockMvcRequestBuilders.get("/detailBoard.do").param("seq", "3"))
			   				.andExpect(MockMvcResultMatchers.status().isOk())
				    		.andReturn();
